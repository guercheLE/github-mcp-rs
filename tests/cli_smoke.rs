...
fn generated_server(args: &[&str]) -> Output {
    Command::new(env!("CARGO_BIN_EXE_github-mcp"))
        .args(args)
        .env("GITHUB_MCP_URL", "http://127.0.0.1")
        .env("GITHUB_MCP_AUTH_METHOD", "pat")
        .env_remove("GITHUB_MCP_API_VERSION")
        .env_remove("GITHUB_MCP_TRANSPORT")
        // Isolates the config cascade's global-config layer
        // (`~/.github-mcp/config.yml`, resolved via
...
fn version_help_versions_and_config_are_available() {
    ...
    assert!(stdout(&versions).contains("gh-2026-03-10"));
    ...
    assert_eq!(config["api_version"], "gh-2026-03-10");
...
fn invalid_commands_operations_and_arguments_fail_without_network_calls() {
    ...
    let invalid_json = generated_server(&["call", "meta/root", "--args", "not-json"]);
...
fn stdio_and_helper_binaries_cover_their_bootstrap_and_error_paths() {
    ...
    let healthy = Command::new(env!("CARGO_BIN_EXE_github-mcp-healthcheck"))
    ...
    let unhealthy = Command::new(env!("CARGO_BIN_EXE_github-mcp-healthcheck"))
        .current_dir(empty.path())
        .output()
        .unwrap();
    assert!(!unhealthy.status.success());

    let populate = Command::new(env!("CARGO_BIN_EXE_github-mcp-populate-embeddings"))
    ...
    .unwrap();
    let populated = Command::new(env!("CARGO_BIN_EXE_github-mcp-populate-embeddings"))
        .arg(&store_copy)
        .output()
        .unwrap();
...
fn http_command_serves_health_and_shuts_down_cleanly() {
    ...
    let mut child = Command::new(env!("CARGO_BIN_EXE_github-mcp"))
        .args([
            "http",
            "--host",
            "127.0.0.1",
